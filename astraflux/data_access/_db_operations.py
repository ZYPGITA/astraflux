...
def _ensure_task_id_exists(task_data: TaskData) -> TaskData:
    ...
    """
    task_id = task_data.get(DEFINITIONS.TASK.TASK_ID)
    if task_id is not None and task_id.strip() != "":
        return task_data

    task_data[DEFINITIONS.TASK.TASK_ID] = snowflake_id()
    return task_data
...
def _build_task_full_data(queue_name: str, task_data: TaskData, weight: int = DefaultValues.TASK.TASK_WEIGHT) \
    ...
    Args:
        ...
        weight (int, optional): Task priority weight (higher = higher priority). Defaults to 
            `TASK.DEFAULT_VALUE_TASK_WEIGHT` (typically 1).
    ...
    System Fields Added:
        - `DEFINITIONS.TASK.TASK_WEIGHT`: Task priority weight.
        - `DEFINITIONS.TASK.TASK_QUEUE_NAME`: Associated queue name.
        - `DEFINITIONS.TASK.TASK_IS_SUB_TASK`: Flag for subtask (always False for main tasks).
        - `DEFINITIONS.TASK.TASK_STATUS`: Initial status (`DEFINITIONS.TASK.TASK_WAIT_STATUS` = "waiting").
        - `DEFINITIONS.TASK.TASK_IS_SUB_TASK_ALL_FINISH`: Subtask completion flag (False initially).
        - `DEFINITIONS.TASK.TASK_CREATE_TIME`: Formatted creation timestamp (via `get_converted_time()`).
    """

    task_data = _ensure_task_id_exists(task_data)

    full_task_data = {
        DEFINITIONS.TASK.TASK_BODY: task_data,
        DEFINITIONS.TASK.TASK_WEIGHT: weight,
        DEFINITIONS.TASK.TASK_QUEUE_NAME: queue_name,
        DEFINITIONS.TASK.TASK_IS_SUB_TASK: False,
        DEFINITIONS.TASK.TASK_ID: task_data[DEFINITIONS.TASK.TASK_ID],
        DEFINITIONS.TASK.TASK_STATUS: DEFINITIONS.TASK.TASK_WAIT_STATUS,
        DEFINITIONS.TASK.TASK_IS_SUB_TASK_ALL_FINISH: False,
        DEFINITIONS.TASK.TASK_CREATE_TIME: get_converted_time()
    }
...
def task_submit_to_db(queue_name: str, task_data: TaskData, weight: int = DefaultValues.TASK.TASK_WEIGHT) -> str:
    ...
    _task_collector.update(
        query={DEFINITIONS.TASK.TASK_ID: full_task_data[DEFINITIONS.TASK.TASK_ID]},
        data=full_task_data,
        upsert=True
    )
    return task_data[DEFINITIONS.TASK.TASK_ID]
...
def subtask_batch_create(source_task_id: str, subtask_queue: str, subtask_list: List[TaskData]) -> List[str]:
    ...
    for subtask_data in subtask_list:
        ...
        subtask_data[DEFINITIONS.TASK.TASK_SOURCE_ID] = source_task_id

        full_subtask_data = {
            DEFINITIONS.TASK.TASK_BODY: subtask_data,
            DEFINITIONS.TASK.TASK_WEIGHT: DefaultValues.TASK.TASK_WEIGHT,
            DEFINITIONS.TASK.TASK_QUEUE_NAME: subtask_queue,
            DEFINITIONS.TASK.TASK_SOURCE_ID: source_task_id,
            DEFINITIONS.TASK.TASK_IS_SUB_TASK: True,
            DEFINITIONS.TASK.TASK_ID: subtask_data[DEFINITIONS.TASK.TASK_ID],
            DEFINITIONS.TASK.TASK_STATUS: DEFINITIONS.TASK.TASK_WAIT_STATUS,
            DEFINITIONS.TASK.TASK_IS_SUB_TASK_ALL_FINISH: False,
            DEFINITIONS.TASK.TASK_CREATE_TIME: get_converted_time()
        }

        subtask_ids.append(subtask_data[DEFINITIONS.TASK.TASK_ID])

        _task_collector = mongodb_get_task_collector()
        _task_collector.update(
            query={DEFINITIONS.TASK.TASK_ID: full_subtask_data[DEFINITIONS.TASK.TASK_ID]},
...
def task_get_by_id(task_id: str) -> Dict[str, Any]:
    ...
    task_data = _task_collector.find_one(
        query={DEFINITIONS.TASK.TASK_ID: task_id},
        fields={'_id': 0}
    )
...
def task_and_subtasks_stop(task_id: str, expire_seconds: int = 604800) -> None:
    ...
    """
    stop_status = {DEFINITIONS.TASK.TASK_STATUS: DEFINITIONS.TASK.TASK_STOP_STATUS}
    ...
    _task_collector.update(
        query={DEFINITIONS.TASK.TASK_SOURCE_ID: task_id},
        data=stop_status
    )

    subtask_id_records = _task_collector.find(
        query={DEFINITIONS.TASK.TASK_SOURCE_ID: task_id},
        fields={DEFINITIONS.TASK.TASK_ID: 1, '_id': 0}
    )

    _redis_task_client = redis_get_task_client()
    for record in subtask_id_records:
        subtask_id = record.get(DEFINITIONS.TASK.TASK_ID)
        if subtask_id:
...
def task_status_get_from_redis(task_id: str) -> Optional[str]:
    ...
    return task_cache.get(DEFINITIONS.TASK.TASK_STATUS)
